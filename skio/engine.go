...
func (engine *realTimeEngine) saveAppSocket(userId int, appSck AppSocket) {
	engine.locker.Lock()
...
func (engine *realTimeEngine) getAppSocket(userId int) []AppSocket {
	engine.locker.RLocker()
	defer engine.locker.RUnlock()

	return engine.storage[userId]
}

func (engine *realTimeEngine) removeAppSocket(userId int, appSck AppSocket) {
	engine.locker.Lock()
...
func (engine *realTimeEngine) Run(appCtx appctx.AppContext, r *gin.Engine) error {
	server, err := socketio.NewServer(&engineio.Options{Transports: []transport.Transport{websocket.Default}})
	...
	server.OnConnect("/", func(s socketio.Conn) error {
		s.SetContext("")
		fmt.Println("connected:", s.ID(), "IP:", s.RemoteHeader(), s.ID())
		return nil
	})
	...
	server.OnEvent("", "authenticate", func(s socketio.Conn, token string) {
		...
		if user.Status == 0 {
			s.Emit("authentication failed", errors.New("you has been banned or deleted"))
			s.Close()
			return
		}
